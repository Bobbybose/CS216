...
double calculateGradebook(const vector<FinalGrade>& gradebook, double& max_score, double& min_score)
{
   ...
   max_score=gradebook[0].getScore();               //Assuming the first score is the largest
   min_score=gradebook[0].getScore();               //Assuming the first score is the smallest
   double average=0;                                //Variable for the average

    for(int i=0; i<gradebook.size(); i++){          //Checking every grade in the gradebook
        average+=gradebook[i].getScore();           //Add the current score to average
        if(max_score<gradebook[i].getScore())       //If the current score is bigger than the current max score
            max_score=gradebook[i].getScore();          //It is the next biggest score
        if(min_score>gradebook[i].getScore())       //If the current score is smaller than the current min score
            min_score=gradebook[i].getScore();          //It is the smallest score
    }

    average=average/gradebook.size();               //Diving the sum of scores by number of scores

...
void IMDB::insert_a_movie(string movieTitle, set<string> actorNames)
{
	...
	if (movies_db.find(movieTitle) == movies_db.end()) {   //If the movie isn't in the map
		movies_db[movieTitle] = actorNames;     //Add the new movie and it's actors
	}
	else {
		set<string>::iterator i;    //Otherwise movie is in the map
		for (i = actorNames.begin(); i != actorNames.end(); i++) {
			movies_db[movieTitle].insert(*i);    //Add each new actor
		}
	}
	...
	set<string>::iterator j;
	for (j = actorNames.begin(); j != actorNames.end(); j++) {
		actors_db[*j].insert(movieTitle);    //Adding the new movie to it's actors
	}
